...
Note: `plan-review.test.ts` **already exists** with 10+ tests and helpers (`makeAgent`, `makeDispatchResult`, `passReviewJson`, `failReviewJson`, `makeCtx`, `makeStateWithPlan`). Add tests to the existing file.
...
- title: Add validationCommand to SuperteamConfig
  description: >
    ...
    optional string field to SuperteamConfig interface and 'tsc --noEmit' default to
    DEFAULT_CONFIG.
  files: [src/config.ts, src/config.test.ts]

- title: Add resetToSha and squashCommitsSince to git-utils.ts
  description: >
    Write tests using temp git repos. Reuse existing makeTempRepo/makeTempDir helpers and
    getCurrentSha import already in git-utils.test.ts. Use the same dynamic import pattern
    for execFile that the existing tests use (await import("node:child_process")). Test
    resetToSha (reset to previous commit, empty SHA, invalid SHA, non-repo) and
    squashCommitsSince (squash multiple commits into one, no-op when baseSha equals HEAD,
    non-repo). Both functions use the existing promisified execFile already at top of
    git-utils.ts.
  files: [src/workflow/git-utils.ts, src/workflow/git-utils.test.ts]
...
- title: Add summary field to TaskExecState
  description: >
    ...
    changedFiles, and that tasks without summary still work. Add the summary type to
    orchestrator-state.ts TaskExecState type.
  files: [src/workflow/orchestrator-state.ts, src/workflow/orchestrator-state.test.ts]
...
- title: Add previousTaskSummary to buildImplPrompt
  description: >
    ...
    When provided, output should contain "## Previous task" section with title, status,
    and changed files. When omitted, no such section. Add optional third parameter to
    buildImplPrompt. Existing callers pass only 2 args so backward compatible.
  files: [src/workflow/prompt-builder.ts, src/workflow/prompt-builder.test.ts]

- title: Add rollback option to escalate in execute.ts
  description: >
    Update the existing vi.mock for git-utils.js (currently mocks getCurrentSha and
    computeChangedFiles) to also include resetToSha. Add mockResetToSha ref. Write tests:
    (1) Rollback is offered when gitShaBeforeImpl is set — verify resetToSha is called with
    saved SHA. (2) Rollback is NOT offered when gitShaBeforeImpl is not set. Import resetToSha
    from git-utils. Update escalate signature to accept cwd and optional gitSha. Build options
    array dynamically — include Rollback only when gitSha is provided. When Rollback selected,
    call resetToSha then return retry. Update all escalate call sites in execute.ts to pass
    ctx.cwd and task.gitShaBeforeImpl where appropriate.
  files: [src/workflow/phases/execute.ts, src/workflow/phases/execute.test.ts]
...
- title: Add onStreamEvent wiring to brainstorm, plan-write, plan-review
  description: >
    ...
    5th arg (onUpdate) and makeOnStream() as 6th arg to all dispatchAgent calls. Note:
    dispatchParallel doesn't support per-agent onStreamEvent — known limitation, no change.
  files: [src/workflow/phases/brainstorm.ts, src/workflow/phases/plan-write.ts, src/workflow/phases/plan-review.ts, src/workflow/phases/brainstorm.test.ts, src/workflow/phases/plan-write.test.ts, src/workflow/phases/plan-review.test.ts]
